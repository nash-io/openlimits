...
async fn order_status() {
    let exchange = init();
    let transaction = exchange.limit_sell("BNBBTC", Decimal::new(1, 1), Decimal::new(2, 3)).await.unwrap();
    let resp = exchange
...
async fn limit_buy() {
    let exchange = init();
    let resp = exchange.limit_buy("BNBBTC", Decimal::new(1, 1), Decimal::new(1, 3)).await.unwrap();
    println!("{:?}", resp);
}
...
async fn limit_sell() {
    let exchange = init();
    let resp = exchange.limit_sell("BNBBTC", Decimal::new(1, 1), Decimal::new(2, 3)).await.unwrap();
    println!("{:?}", resp);
}
...
async fn market_buy() {
    let exchange = init();
    let resp = exchange.market_buy("BNBBTC", Decimal::new(1, 1)).await.unwrap();
    println!("{:?}", resp);
}
...
async fn market_sell() {
    let exchange = init();
    let resp = exchange.market_sell("BNBBTC", Decimal::new(1, 1)).await.unwrap();
    println!("{:?}", resp);
}
